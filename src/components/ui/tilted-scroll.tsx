      ...
      <div className="relative overflow-hidden [mask-composite:intersect] [mask-image:linear-gradient(to_right,transparent,black_5rem),linear-gradient(to_left,transparent,black_5rem),linear-gradient(to_bottom,transparent,black_5rem),linear-gradient(to_top,transparent,black_5rem)]">
        <div className="grid h-[250px] w-[300px] gap-5 animate-skew-scroll grid-cols-1">
          {items.map((item) => (
            <div
              key={item.id}
              className="group flex items-center gap-2 cursor-pointer rounded-md border border-green-400/40 bg-gradient-to-b from-gray-900/80 to-gray-800/80 p-4 shadow-md transition-all duration-300 ease-in-out hover:scale-105 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-xl hover:border-green-400/60"
            >
              <CheckCircleIcon className="h-6 w-6 mr-2 stroke-green-400/60 transition-colors group-hover:stroke-green-400" />
...
const defaultItems: TiltedScrollItem[] = [
  ...
  { id: "7", text: "Expert Team" },
  { id: "8", text: "Innovation First" },
]